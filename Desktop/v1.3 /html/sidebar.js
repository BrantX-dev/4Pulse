...
function renderTopics(favoritesData) {
    ...
    const all  = settings.show_all_favorites ? favoritesData.list : favoritesData.list.filter(t => !t.viewed);

    // 🎯 Focused topics always first — same as popup
    const focusedUnread = all.filter(t => sidebarFocusedTopics.has(String(t.id)));
    const normalUnread  = all.filter(t => !sidebarFocusedTopics.has(String(t.id)));
    const ordered = [...focusedUnread, ...normalUnread];

    ordered.forEach((t, i) => frag.appendChild(createTopicCard(t, tpl, i, false)));
    elements.topicsList.innerHTML = '';
    elements.topicsList.appendChild(frag);

    // Toggle has-focused class for dimming non-focused cards (same CSS as popup)
    elements.topicsList.classList.toggle('has-focused', focusedUnread.length > 0);
}
